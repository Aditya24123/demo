...
export function normalizeWorkspace(data: any, isFallback = false): WorkspaceVM {
  ...
  const description =
    ...
    data.description ||
    null;
  const capabilities = data.capabilities || summary.capabilities || null;
  const mpMaterialId = summary.mp_material_id || material.mp_material_id || data.mp_material_id || null;
  ...
  return {
    ...
    mpMaterialId: mpMaterialId ? String(mpMaterialId) : null,
    raw: data,
  };
}
...
export function normalizeProject(project: any): ProjectVM {
  return {
    projectId: String(project?.project_id || ''),
    name: String(project?.name || project?.project_id || 'Untitled project'),
...
export function normalizeSession(s: any): SessionVM {
  ...
  const title = typeof rawTitle === 'string'
    ? rawTitle
    : rawTitle && typeof rawTitle.id === 'string'
      ? rawTitle.id
      : String(s.session_id || s.id || 'Untitled session');
  ...
  return {
    ...
    messageCount: Number(s.message_count ?? s.messages?.length ?? 0),
    projectId: context.project_id ? String(context.project_id) : s.project_id ? String(s.project_id) : null,
    archived: Boolean(context.archived ?? s.archived),
    materialId: context.current_material_id || context.material_id || s.current_material_id || null,
  };
}
