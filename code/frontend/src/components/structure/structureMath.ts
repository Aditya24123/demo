...
/**
 * Bond inference inside the visible unit-cell sites only.
 * Periodic image bonds were drawing dangling sticks into empty space ? disable that for display.
 * Minimum-image distance is still used to *decide* if two sites are bonded under PBC,
 * but the drawn segment always connects the real atom positions in the cell.
 */
export function computeBondSegments(
  sites: ParsedSite[],
  options: { maxBonds?: number; latticeVectors?: Vec3[]; periodic?: boolean } = {},
): BondSegment[] {
  const maxBonds = options.maxBonds ?? 900;
  const lattice = options.latticeVectors || [];
  const useMic = Boolean(options.periodic && lattice.length === 3);
  ...
  for (let i = 0; i < sites.length; i += 1) {
    for (let j = i + 1; j < sites.length; j += 1) {
      const a = sites[i];
      const b = sites[j];
      const cutoff = Math.min(DEFAULT_BOND_MAX, (a.radius + b.radius) * 1.25);
      let dist = Math.hypot(
      ...
      if (useMic) {
        // Minimum-image distance for bond decision only (no off-cell sticks).
        let best = dist;
        for (let ii = -1; ii <= 1; ii += 1) {
          for (let jj = -1; jj <= 1; jj += 1) {
            for (let kk = -1; kk <= 1; kk += 1) {
              if (ii === 0 && jj === 0 && kk === 0) continue;
              const off = add(add(mul(lattice[0], ii), mul(lattice[1], jj)), mul(lattice[2], kk));
              const d = Math.hypot(
                a.position[0] - (b.position[0] + off[0]),
                a.position[1] - (b.position[1] + off[1]),
                a.position[2] - (b.position[2] + off[2]),
              );
              if (d < best) best = d;
            }
          }
      ...
      if (dist > 0.2 && dist <= cutoff) {
        bonds.push({ a: a.position, b: b.position, aElement: a.element, bElement: b.element });
