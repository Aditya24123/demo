...
export function UnitCell({ vectors, color = '#a8b3c4' }: UnitCellProps) {
  ...
  return (
    <group>
      {edges.map((edge, idx) => (
        <Line key={idx} points={edge as [number, number, number][]} color={color} lineWidth={1.4} transparent opacity={0.82} />
      ))}
    </group>
  );
}
