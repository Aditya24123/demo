...
import type { GraphNodeVM } from '@/catalyst/bridge/viewModels'
import type {
  GraphCanvasProps,
  GraphMethods,
  RenderLink,
...
export function GraphCanvas({ graphOverride }: GraphCanvasProps = {}) {
  ...
  const { openSheet, theme, setInspectorOpen, setInspectorTab } = useCatalystLayout()
  ...
  const handleNodeClick = useCallback((node: RenderNode) => {
    ...
    // Live shell uses inspectorOpen; legacy openSheet kept for older sheets only
    setInspectorOpen(true)
    setInspectorTab('properties')
    openSheet('inspector')
    fgRef.current?.centerAt?.(node.x ?? 0, node.y ?? 0, 550)
    fgRef.current?.zoom?.(2.4, 550)
  }, [openSheet, selectGraphNode, setInspectorOpen, setInspectorTab])
  ...
  const paintNode = useCallback((node: RenderNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    ctx.font = `${tier === 0 ? 700 : tier === 1 ? 600 : 500} ${fontSize}px -apple-system-body, ui-sans-serif, -apple-system, system-ui, "Segoe UI", Helvetica, Arial, sans-serif`
  ...
  const paintLinkLabel = useCallback((link: RenderLink, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    ctx.font = `500 ${fs}px -apple-system-body, ui-sans-serif, -apple-system, system-ui, "Segoe UI", Helvetica, Arial, sans-serif`
    ctx.textAlign = 'center'
  ...
  return (
    ...
    >
      ...
      {contextMenu && (
        ...
        >
          <button className="w-full text-left px-3 py-1.5 hover:bg-[var(--surface-2)]" onClick={() => { selectGraphNode(contextMenu.node.id); setInspectorOpen(true); setInspectorTab('properties'); openSheet('inspector'); setContextMenu(null); }}>Inspect {contextMenu.node.type}</button>
