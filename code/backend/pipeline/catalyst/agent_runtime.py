...
DEFAULT_AGENT_CONTEXT: dict[str, Any] = {
    ...
    "persona": {
        ...
        "rules": [
            ...
            "After select_material or get_material_structure, say you requested the UI focus/tab change ? the client applies ui_actions. Do not claim the viewer already shows the material unless the tool returned ok.",
            "If a tool returns ok:false or material not found, say so clearly and do not invent success.",
            "Do not advertise property prediction models unless list_model_services shows configured services.",
        ],
    },
    "runtime": {
...
def render_agent_context_markdown(payload: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## LIVE VIEWPORT (authoritative ? wins over chat history)"])
    live = dynamic_context.get("live_viewport") or {}
    if isinstance(live, dict) and (live.get("material_id") or live.get("project_id")):
        mid = live.get("material_id") or "none"
        formula = live.get("formula_pretty") or "unknown"
        lines.append(f"- Open material: **{formula}** (`{mid}`)")
        if live.get("chemsys"):
            lines.append(f"- Chemsys: {live.get('chemsys')}")
        if live.get("workspace_tab"):
            lines.append(f"- Active tab: {live.get('workspace_tab')}")
        if live.get("project_id"):
            lines.append(f"- Project: {live.get('project_name') or live.get('project_id')}")
        lines.append(
            "- Never claim the user is viewing a different material just because earlier messages mentioned one."
        )
    else:
        lines.append("- No material currently open in the live viewport.")
    guidance = str(dynamic_context.get("mode_guidance") or "").strip()
    if guidance:
        lines.extend(["", f"Mode guidance: {guidance}"])
    lines.extend(["", "## Dynamic Catalyst Context", "```json"])
    surface = str(dynamic_context.get("agent_surface") or "").strip()
    if surface:
        lines.append(f"Active agent surface: {surface}")
    # Prefer a compact dump: live first, then workspace, then session ? omit huge message bodies here
    compact_dynamic = {
        "live_viewport": dynamic_context.get("live_viewport"),
        "agent_surface": dynamic_context.get("agent_surface"),
        "current_workspace": dynamic_context.get("current_workspace"),
        "session": dynamic_context.get("session"),
    }
    lines.append(json.dumps(compact_dynamic, indent=2, sort_keys=True))
    lines.append("```")
    return "\n".join(lines).strip()
