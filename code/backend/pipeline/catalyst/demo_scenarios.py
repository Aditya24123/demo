...
class DemoScenario:
    ...
    def mission(self) -> list[dict[str, str]]:
        return [{"id": step.key, "label": step.label} for step in self.steps if step.key != "variant-focus"]


SUNLIGHT_BRIEF = """## Sunlight Protection Science Brief

- **Material shield:** Zinc oxide (ZnO, local record `mp-deb`) is a mineral UV-blocking sunscreen material. Its stored atomic coordinates assemble into the structure shown here.
- **Evidence:** Catalyst connects the material record to its local structure and evidence graph; this is an inspected record, not a newly discovered material.
...
- **Safety:** Sun protection reduces harmful UV exposure. This demonstration does **not** claim sunlight caused the displayed variant and is not a diagnosis or clinical interpretation.
"""
...
SUNLIGHT_SCENARIO = DemoScenario(
    ...
    steps=(
        DemoStep(
            ...
            hold_status="Framing the investigation?",
        ),
        DemoStep(
            key="find-material",
            label="Find material",
            ...
            narration=(
                ...
                "ZnO is used as a mineral UV blocker in sunscreen; the identifier and structure shown next "
                "come from the cached local record.\n\n"
            ),
            ui_actions=(
        ...
        DemoStep(
            ...
            narration=(
                "The atoms begin scattered for orientation, then move deterministically into the **real stored "
                "coordinates**. Once they settle, the inferred bonds and lattice frame appear.\n\n"
            ),
            ui_actions=(
                {"type": "demo_checkpoint", "step_id": "assemble-atoms"},
                {"type": "set_workspace_tab", "tab": "structure"},
                {"type": "demo_material_assemble", "duration_ms": 26_000, "nonce": 1},
            ),
            hold_status="Atoms are aligning and settling?",
        ),
        DemoStep(
            key="evidence-graph",
            ...
            narration=(
                "Now Catalyst opens the material neighborhood. This evidence view connects the ZnO record to "
                "its local graph context without claiming a new discovery.\n\n"
            ),
            ui_actions=(
                {"type": "demo_checkpoint", "step_id": "evidence-graph"},
                {"type": "expand_neighborhood", "material_id": "mp-deb", "depth": 1},
                {"type": "set_workspace_tab", "tab": "neighbors"},
            ),
            hold_status="Tracing cached material relationships?",
        ),
        DemoStep(
            key="dna-repair",
            label="DNA repair",
            ...
            narration=(
                ...
                "repair. The visible DNA window enters as four fragments, rotates into alignment, and connects "
                "into the existing helix.\n\n"
            ),
            ui_actions=(
                ...
                {"type": "demo_dna_assemble", "duration_ms": 24_000, "nonce": 1},
            ),
            hold_status="DNA fragments are rotating into alignment?",
        ),
        DemoStep(
            key="variant-focus",
            label="DNA repair",
            ...
            narration=(
                ...
                "This is educational context?not evidence that sunlight caused this variant and not a diagnosis.\n\n"
            ),
            ui_actions=(
                {"type": "demo_dna_focus", "duration_ms": 10_000, "nonce": 2},
