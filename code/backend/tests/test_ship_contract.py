...
def _install_fake_gemma_agent(monkeypatch, mapping: dict[str, dict]) -> None:
    ...
    monkeypatch.setattr("catalyst.agent.loops.generate_gemini_agent_turn", fake_turn)
...
def test_session_store_tolerates_corrupt_file_and_delete_endpoint() -> None:
    sessions = SessionStore(REPO_ROOT)
    corrupt_path = sessions.root / "ses_corrupt_contract.json"
    corrupt_path.write_text("", encoding="utf-8")
    assert sessions.get_session("ses_corrupt_contract") is None
    assert all(item["session_id"] != "ses_corrupt_contract" for item in sessions.list_sessions())

    client = TestClient(app)
    session = client.post("/sessions", json={"title": "Delete contract"}).json()
    deleted = client.delete(f"/sessions/{session['session_id']}")
    assert deleted.status_code == 200
    assert deleted.json()["status"] == "deleted"
    missing = client.get(f"/sessions/{session['session_id']}")
