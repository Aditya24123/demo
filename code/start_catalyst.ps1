...
$runtimeConfigPath = Join-Path $frontendRoot "public\runtime-config.json"
$remoteApiUrl = $env:CATALYST_API_BASE_URL
if (-not $remoteApiUrl -and (Test-Path -LiteralPath $runtimeConfigPath)) {
    try {
        $runtimeConfig = Get-Content -LiteralPath $runtimeConfigPath -Raw | ConvertFrom-Json
        if ($runtimeConfig.apiBaseUrl) {
            $remoteApiUrl = [string]$runtimeConfig.apiBaseUrl
        }
    } catch {
        $remoteApiUrl = $null
    }
...
$apiUrl = if ($remoteApiUrl) { $remoteApiUrl.TrimEnd("/") } else { "http://127.0.0.1:8766" }
$frontendUrl = "http://127.0.0.1:5173"
$remoteBackend = -not [string]::IsNullOrWhiteSpace($remoteApiUrl)
$reuseBackend = $false
if ($remoteBackend) {
    try {
        $health = Invoke-RestMethod -Method Get -Uri "$apiUrl/health" -TimeoutSec 5
        $reuseBackend = ($health.status -eq "ok")
    } catch {
        $reuseBackend = $false
    }
    if (-not $reuseBackend) {
        Write-Host "Remote backend unavailable: $apiUrl"
        exit 1
    }
    Write-Host "Preflight: ok"
    Write-Host "  remote_backend: ok ($apiUrl)"
} else {
    try {
...
$backend = $null
if (-not $remoteBackend -and -not $reuseBackend) {
