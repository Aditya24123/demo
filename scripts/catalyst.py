...
def configured_api_base(root: Path) -> str:
    env_value = os.getenv("CATALYST_API_BASE_URL", "").strip()
    if env_value:
        return env_value.rstrip("/")

    config_path = root / "code" / "frontend" / "public" / "runtime-config.json"
    ...
    try:
        ...
        return ""
    value = str(config.get("apiBaseUrl") or "").strip()
    return value.rstrip("/")
...
def main() -> int:
    ...
    local_api_url = f"http://127.0.0.1:{args.api_port}"
    ui_url = f"http://127.0.0.1:{args.ui_port}"
    remote_api_url = configured_api_base(root)
    api_url = remote_api_url or local_api_url
    ...
    if remote_api_url:
        status("backend mode", "remote", remote_api_url)
        if not wait_for_url(f"{remote_api_url}/health", label="backend health", seconds=15):
            raise SystemExit(f"Remote backend did not become healthy: {remote_api_url}")
        if args.check or args.install_only:
            status("frontend setup", "ok")
            return 0
    else:
    ...
    try:
        if remote_api_url:
            status("backend", "remote", remote_api_url)
        elif check_url(f"{local_api_url}/health"):
            status("backend", "reuse", local_api_url)
        else:
            ...
            status("backend", "start", local_api_url)
            if not wait_for_url(f"{local_api_url}/health", label="backend health", seconds=90):
