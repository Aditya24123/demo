...
## Current Mini Development Topology

The active local-development frontend keeps all processed data and provider
execution on `mini`:

```text
http://127.0.0.1:5173                local Vite frontend
https://mini.tail3bfb03.ts.net:8766  FastAPI and /voice/live
https://mini.tail3bfb03.ts.net       static UI snapshot
```

Tailscale Serve proxies port `8766` to the loopback-bound FastAPI process. The
static snapshot on port `443` remains independent, so frontend deployment does
not restart the API or touch `data/`, `.secrets/`, or provider settings.

The mini API must be launched with:

```bash
/home/ubuntu/catalyst-live/start-catalyst-live.sh
```

That script activates the project virtual environment, sets `CATALYST_REPO_ROOT`
and `PYTHONPATH`, binds the API to loopback, and reads `GEMINI_API_KEY` from the
server-side `.secrets/gemini_api_key` file. Starting uvicorn manually omits the
voice/provider secret and makes Gemini appear unavailable.

The Codex workspace runner lives in `code/backend/codex-service/` and uses the
official `@openai/codex-sdk`. Keep Node, package dependencies, and Codex login
available on the mini host. Scientific model credentials follow the same
server-only environment-variable pattern; the settings file stores only each
credential's environment name.
