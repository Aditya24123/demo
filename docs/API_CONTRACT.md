...
GET /projects/{project_id}/notebook
PUT /projects/{project_id}/notebook
GET /projects/{project_id}/files/{file_path}
PUT /projects/{project_id}/files/{file_path}
GET /projects/{project_id}/runs
POST /projects/{project_id}/codex/run
```
...
```ts
{
  ...
  model_services?: Record<string, ModelServiceSettings>;
}
```
...
Provider settings support built-in Gemini, Groq, Mistral, NVIDIA, Ollama Cloud,
and Ollama entries plus custom OpenAI-compatible base URLs. `api_key_envs` stores
environment-variable names only.

Scientific model services are preconfigured by an operator. Agents select a
`service_id` and structured `inputs`; they cannot supply arbitrary URLs. Each
service defines an HTTP(S) endpoint, task, optional model, optional API-key
environment name, enabled state, and 5-900 second timeout. Responses must be
JSON and are capped at 5 MB.

## Project Workspaces

Each project is a real folder under `data/local/projects/<project_id>/` with a
primary Markdown notebook, bounded UTF-8 files, artifacts, persistent agent
state, and run history. Editable paths are restricted to `files/`, `notebook/`,
and `artifacts/`; path traversal and files larger than 2 MB are rejected.

`POST /projects/{project_id}/codex/run` executes the official Codex SDK inside
that project folder with workspace-write permissions and approval mode `never`.
The backend persists the Codex thread id, final response, usage, status, and
errors as project runs.
...
The live catalog exposes 26 tools across materials search and graph inspection,
candidate workflows, research, project files and notebooks, Codex workspace
runs, structure/details retrieval, and configured scientific model services.
Tool results are authoritative: the response layer must not claim a workspace
mutation succeeded when the tool returned `ok: false`, an error, or an empty
result.

`WS /voice/live` uses the same tool declarations and workspace context as text
chat. The client sends a `start` frame first, then receives `ready` and
`setup_complete` before streaming PCM audio or text frames.
